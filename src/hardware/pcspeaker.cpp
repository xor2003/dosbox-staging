...
float NeutralOr(float fallback)
{
...
float NeutralLastPitOr(float fallback)
{
	return NeutralOr(std::isgreater(fabs(spkr.pit_last), SPKR_NEUTRAL_VOLTAGE)
	                         ? spkr.pit_last
	                         : fallback);
}
...
public:
	PCSPEAKER(Section *configuration) : Module_base(configuration)
	{
		...
		spkr.chan->SetPeakMagnitude(static_cast<uint32_t>(SPKR_POSITIVE_VOLTAGE));
	}
	~PCSPEAKER(){
