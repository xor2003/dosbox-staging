...
bool MidiHandlerFluidsynth::Open(const char *const /*conf*/)
{
	LocaleGuard locale_guard;

	Close();

	auto *section = static_cast<Section_prop *>(
...
void MidiHandlerFluidsynth::PlayMsg(Bit8u *msg)
{
...
void MidiHandlerFluidsynth::PlaySysex(Bit8u *sysex, Bitu len)
{
	const char *data = reinterpret_cast<const char *>(sysex);
	fluid_synth_sysex(synth.get(), data, len, nullptr, nullptr,
	                  nullptr, false);
}

void MidiHandlerFluidsynth::mixerCallback(const Bitu len)
{
	fluid_synth_write_s16(instance.synth.get(), len, MixTemp, 0, 2,
	                      MixTemp, 1, 2);
	const int16_t *data = reinterpret_cast<int16_t *>(MixTemp);
