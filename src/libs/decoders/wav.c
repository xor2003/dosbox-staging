...
static const char *extensions_wav[] = { "WAV", "WAVE", NULL };
...
static int read_fmt_chunk(SDL_RWops *rw, fmt_t *fmt)
{
    ...
    BAIL_IF_MACRO(!read_le32(rw, (Uint32*) &fmt->chunkSize), NULL, 0);
    BAIL_IF_MACRO(fmt->chunkSize < 16, "WAV: Invalid chunk size", 0);
    fmt->next_chunk_offset = SDL_RWtell(rw) + fmt->chunkSize;
    
    BAIL_IF_MACRO(!read_le16(rw, (Uint16*) &fmt->wFormatTag), NULL, 0);
    BAIL_IF_MACRO(!read_le16(rw, (Uint16*) &fmt->wChannels), NULL, 0);
    BAIL_IF_MACRO(!read_le32(rw, (Uint32*) &fmt->dwSamplesPerSec), NULL, 0);
    BAIL_IF_MACRO(!read_le32(rw, (Uint32*) &fmt->dwAvgBytesPerSec), NULL, 0);
    BAIL_IF_MACRO(!read_le16(rw, (Uint16*) &fmt->wBlockAlign), NULL, 0);
    BAIL_IF_MACRO(!read_le16(rw, (Uint16*) &fmt->wBitsPerSample), NULL, 0);
...
static int read_data_chunk(SDL_RWops *rw, data_t *data)
{
    /* skip reading the chunk ID, since it was already read at this point... */
    data->chunkID = dataID;
    BAIL_IF_MACRO(!read_le32(rw, (Uint32*) &data->chunkSize), NULL, 0);
    return(1);
} /* read_data_chunk */
...
static __inline__ int read_adpcm_block_headers(Sound_Sample *sample)
{
    ...
    for (i = 0; i < max; i++)
        BAIL_IF_MACRO(!read_uint8(rw, (Uint8*) &headers[i].bPredictor), NULL, 0);

    for (i = 0; i < max; i++)
        BAIL_IF_MACRO(!read_le16(rw, (Uint16*) &headers[i].iDelta), NULL, 0);

    for (i = 0; i < max; i++)
        BAIL_IF_MACRO(!read_le16(rw, (Uint16*) &headers[i].iSamp1), NULL, 0);

    for (i = 0; i < max; i++)
        BAIL_IF_MACRO(!read_le16(rw, (Uint16*) &headers[i].iSamp2), NULL, 0);
...
static int read_fmt_adpcm(SDL_RWops *rw, fmt_t *fmt)
{
    ...
    BAIL_IF_MACRO(!read_le16(rw, (Uint16*) &fmt->fmt.adpcm.cbSize), NULL, 0);
    BAIL_IF_MACRO(!read_le16(rw, (Uint16*) &fmt->fmt.adpcm.wSamplesPerBlock), NULL, 0);
    BAIL_IF_MACRO(!read_le16(rw, (Uint16*) &fmt->fmt.adpcm.wNumCoef), NULL, 0);
    ...
    {
        BAIL_IF_MACRO(!read_le16(rw, (Uint16*) &fmt->fmt.adpcm.aCoef[i].iCoef1), NULL, 0);
        BAIL_IF_MACRO(!read_le16(rw, (Uint16*) &fmt->fmt.adpcm.aCoef[i].iCoef2), NULL, 0);
    } /* for */
...
static int find_chunk(SDL_RWops *rw, Uint32 id)
{
    ...
    while (1)
    {
        BAIL_IF_MACRO(!read_le32(rw, (Uint32*) &_id), NULL, 0);
        if (_id == id)
            return(1);

            /* skip ahead and see what next chunk is... */
        BAIL_IF_MACRO(!read_le32(rw, (Uint32*) &siz), NULL, 0);
        assert(siz >= 0);
